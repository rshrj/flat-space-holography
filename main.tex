...
\section{Minkowski space in Bondi gauge}
   The Bondi or in this case the double null gauge was alluded to in the last section in the context of AdS. Here we study $\mathbb{R}^{3, 1}$ in these coordinates. These are related to global intertial coordinates as follows ($r = \sqrt{(x^1)^2 + (x^2)^2 + (x^3)^2}$):
   \parbox{0.5\linewidth}{\begin{align}
      u &= x^0 - r \cr
      v &= x^0 + r \cr
      z &= \frac{x^1 + i x^2}{r - x^3} = \cot\frac{\theta}{2} e^{i \phi} \cr
      \bar{z} &= \frac{x^1 - i x^2}{r - x^3} = \cot\frac{\theta}{2} e^{-i \phi}
   \end{align}}%
   \parbox{0.5\linewidth}{\begin{align}
      x^0 &= \frac{u + v}{2} \cr
      x^1 &= \frac{(v-u) \, \Re{\,z}}{1 + z \bar{z}} \cr
      x^2 &= \frac{(v-u) \, \Im{\,z}}{1 + z \bar{z}} \cr
      x^3 &= \left( \frac{v-u}{2} \right) \frac{1 - z \bar{z}}{1 + z \bar{z}}
   \end{align}}   

   The metric reads
   \begin{equation}
      \d{s}^2 = -\d{u} \, \d{v} + \left( \frac{v - u}{2} \right)^2 \frac{4 \d{z} \, \d{\bar{z}}}{(1 + z \bar{z})^2}
   \end{equation}
   Note that $z$ is simply the inverse stereographic projection from the North pole.
